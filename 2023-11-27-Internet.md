...
<br>
 - 1.1.1.1.1: Not a valid IP address. An IP address consists of four sets of numbers ranging from 0 to 255, separated by periods. This format doesn't conform to the standard IPv4 structure.
 - 23.23.23.23: Possible. It's a valid IPv4 address following the correct format.
 - 134.492.100.0: Not possible. The second number in the address (492) exceeds the valid range (0-255).
 - 255.256.55.255: Not possible. The second number in the address (256) exceeds the valid range (0-255).
 - 2.93.255.19: Possible. It's a valid IPv4 address following the correct format.
...
- Unicast
- Broadcast
it's an example of broadcast. Broadcast communication sends data to all devices on the network.
...
1. Three of the four following protocols are on the same layer. Identify which ones and what layer they are on, and why they are on each layer:
    - ASCII (see above for information):  Not a protocol but a character encoding standard. It's not associated with a particular layer in the OSI model.
    - FTP (facilitates transfer of files over the internet): Operates at the Application Layer (Layer 7) as it deals with file transfer functions.
    - TLS (see HTTPS section): Functions at the Transport Layer (Layer 4), providing secure communication between applications.
    - USB (permits data exchange between electronics): Not a network protocol; it's a physical interface that permits data exchange between electronic devices.
...
Telnet operates at the Application Layer (Layer 7) of the OSI model. This layer facilitates user interfaces and network applications.
#### DNS
...
Bob should buy the domain best.com. The subdomains would be bob.is.the.
#### HTTP and HTTPS
1. What is a difference between HTTP and HTTPS? : HTTPS (HyperText Transfer Protocol Secure) encrypts data, while HTTP does not.
<br>
 - What protocol does HTTPS use that HTTP doesn't?: HTTPS uses SSL/TLS (Secure Sockets Layer/Transport Layer Security), providing encryption and authentication.

2. Last trimester we sent HTTP requests for our passion projects
<br>
 - Did we use HTTP or HTTPS?: HTTP
 - What are the benefits and disadvantages of this?: Advantages are that it is faster as https does ssl tsl encyrpytion.Disadvantages are lack of security and encryption, making data vulnerable to interception.
...
Bob should use UDP for real-time streaming because it's fast but doesn't check for errors or resend lost data, which might cause some packets to be lost or create errors in the stream.
...
<br>
 ...
 TCP's error checking ensures data integrity during transmission by using acknowledgment and retransmission. For instance, if a packet is lost, TCP retransmits it until it's successfully received.
...
<br>
 ...
 Server B sends acknowledgment (ACK) packets to Server A for each received packet, ensuring their successful reception before Server A continues sending more data.
Another use of acknowledgment in TCP is for flow control, regulating the rate of data transmission between Server A and Server B.In UDP, there's no built-in acknowledgment system. Server B doesn't explicitly confirm received packets. There's no assurance that Server A's packets are received before Server A continues sending more data. There isn't an additional use of acknowledgment in UDP because it does not have the feauture.